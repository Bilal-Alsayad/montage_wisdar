...
export default function AlarabyTemplate({
  ...
  const fontsLoaded = useLoadFonts([
    {
      ...
      url: staticFile("alaraby/fonts/AlarabyTelevisionBold.ttf"),
    },
  ]);
  ...
  return (
    <AbsoluteFill>
      ...
      {data.speakers.length > 0 &&
        data.speakers.map((speaker, index) => (
          <Sequence
            ...
            from={TAGS_ANIMATION_DURATION + TITLE_ANIMATION_DURATION}
            key={index}
      ...
      <Img
        src={staticFile("alaraby/images/logo.png")}
        style={{
          position: "absolute",
          top: 180,
          left: 100,
      ...
      {data.captions.src && (
        <Captions
          ...
          containerStyle={{
            top: 1300,
            backgroundColor: "rgba(204, 47, 47, 0.54)",
      ...
      <Sequence from={outroStartFrame} durationInFrames={outroDurationInFrames}>
        <OffthreadVideo
          src={staticFile("alaraby/elements/outro.webm")}
          transparent
          style={{
            width: "100%",
            height: "100%",
