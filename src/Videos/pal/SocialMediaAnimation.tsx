...
export const PAL_SOCIAL_MEDIA_DURATION = 196;
...
const smoothInOut = Easing.bezier(0.33, 0, 0.67, 1);
...
type SocialMediaAnimationProps = {
  fontFamily?: string;
};
...
export default function SocialMediaAnimation({
  ...
  const introOpacity = interpolate(frame, [0, 5], [0, 1], clamp);
  const outerBorderColor = interpolateColors(
    frame,
    [0, 9],
    ["#303030", "#ffffff"],
  );
  const openingBarWidth = interpolate(frame, [11, 28], [0, BAR_MAX_WIDTH], {
    ...clamp,
    easing: smoothOut,
  });
  const closingBarWidth = interpolate(frame, [174, 185], [BAR_MAX_WIDTH, 0], {
    ...clamp,
    easing: smoothInOut,
  });
  ...
  const textTranslateX = interpolate(frame, [23, 36], [-10, 0], {
    ...clamp,
    easing: smoothOut,
  });
  const textOpacity = interpolate(frame, [23, 33], [0, 1], clamp);
  const textColor = interpolateColors(frame, [24, 37], ["#c5c5c5", "#111111"]);
  const iconMaskWidth = interpolate(frame, [186, 195], [ICON_SIZE, 0], {
    ...clamp,
    easing: smoothInOut,
  });
  const finalOpacity = interpolate(frame, [193, 195], [1, 0], clamp);
