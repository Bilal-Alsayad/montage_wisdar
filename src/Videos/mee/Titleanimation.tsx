...
const STANDARD_EASING = Easing.bezier(0.167, 0.167, 0.833, 0.833);
const MASK_EASING = Easing.bezier(0.167, 0.167, 0, 1);
const TEXT_POSITION_EASING = Easing.bezier(0.072, 0.503, 0.288, 1);
const LINE_SPACING_EASING = Easing.bezier(0.245, 0.972, 0.426, 1);
const TEXT_REVEAL_EASING = Easing.bezier(0.186, 0.808, 0.667, 1);
...
const invertedPolygonPath = (points: readonly Point[]) => {
...
const createExitMask = (progress: number) => {
  if (progress <= 0) return "linear-gradient(#000, #000)";
  if (progress >= 1) return "linear-gradient(transparent, transparent)";
...
export default function TitleAnimation({ text, fontFamily }: TitleAnimationProps) {
  ...
  const animationFrame = Math.max(0, timelineFrame - WHITE_RECTANGLES_DURATION);
  ...
  // Siyah gölge frame 0-37.2'de beyaz ile beraber hareket et,
  ...
  const orangeStartIndex = Math.round(words.length * 0.45);
  const orangeWordCount = Math.max(1, words.length - orangeStartIndex);

  const titleScale =
    title.length > 58 ? 56 / 88 : title.length > 38 ? 68 / 88 : 1;

  const titleFontSize = 88 * titleScale;
  const titleLineHeight = (105.6 + lineSpacing) * titleScale;
    ...
    >
      ...
      >
        ...
        >
          <g
            transform={`
              translate(0 ${COMP_CENTER_Y * (1 - whiteRectanglesScaleY)})
              scale(1 ${whiteRectanglesScaleY})
            `}
          >
            <path
              d={invertedPolygonPath(whiteMaskPoints)}
              fill="#FFFFFF"
              fillRule="evenodd"
              clipRule="evenodd"
            />
            ...
            <polygon
              points={polygonPoints(darkMaskPoints)}
              fill="#000000"
              fillOpacity={darkMaskOpacity}
            />
          </g>
        </svg>
      </div>
      ...
      <div
        style={{
          ...
          maskImage: createEnterMask(enterReveal),
          WebkitMaskImage: createEnterMask(enterReveal),
      ...
      >
        <div
          style={{
            width: "100%",
            height: "100%",
            maskImage: createExitMask(exitReveal),
            WebkitMaskImage: createExitMask(exitReveal),
            maskRepeat: "no-repeat",
            WebkitMaskRepeat: "no-repeat",
          }}
        >
          ...
          >
            {words.map((word, index) => {
              ...
              const orangeIndex = index - orangeStartIndex;

              const stagger =
                orangeWordCount <= 1
                  ? 0
                  : (orangeIndex / (orangeWordCount - 1)) * 0.18;

              const wordColorProgress = isOrange
                ? interpolate(
                    colorProgress,
                    [stagger, Math.min(1, stagger + 0.82)],
                    [0, 1],
                    CLAMP,
                  )
                : 0;
